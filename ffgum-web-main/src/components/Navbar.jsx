...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed w-full z-50 ${scrolled ? 'border-b border-gray-900' : ''} bg-black shadow-md`}>
            <div className="container mx-auto px-4 py-2 flex justify-between items-center">
                ...
                <div className={`flex flex-col md:flex-row items-center justify-center md:justify-between w-full md:w-auto space-y-4 md:space-y-0 md:space-x-4 ml-auto ${menuOpen ? 'block min-h-screen' : 'hidden'} md:flex`}>
                    ...
                    {data?.ffId && (
                        <>
                            <NavItem href="/dashboard/surveys">Gana</NavItem>
                            <NavItem href="/dashboard/withdraw">Retira</NavItem>
                        </>
                    )}
                    <NavItem href="https://discord.gg/kCYGJEPmaE">Soporte</NavItem>
                    <div className="flex items-center space-x-4">
                        <NavButton href={data?.ffId ? "/dashboard" : "/sign-in"}>
                            {data?.ffId ? "Dashboard" : "Acceder"}
                        </NavButton>
                        {data?.ffId && (
                            <NavButtonClose 
                                onClick={clearData} 
